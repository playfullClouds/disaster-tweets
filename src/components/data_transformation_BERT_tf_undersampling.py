...
class DataTransformationConfig:
    ...
    destination_dir: str = os.path.join('artifacts', 'data_transformation', 'BERT_UnderSampling')
...
class DataTransformer:
    ...
    def apply_undersampling(self, input_ids, targets):
        try:
            rus = RandomUnderSampler(random_state=42)
            input_ids_resampled, targets_resampled = rus.fit_resample(input_ids, targets)
            log.info("Applied under-sampling successfully")
            return input_ids_resampled, targets_resampled
        except Exception as e:
            log.error("Failed to apply under-sampling.")
            raise CustomException(e, sys)

    def transform_data(self, df, apply_undersampling=False):
        try:
            texts = df['cleanText'].values
            targets = df['target'].values
            input_ids, attention_masks = self.tokenize_sentences(texts)
            if apply_undersampling:
                input_ids, targets = self.apply_undersampling(input_ids, targets)
                attention_masks, _ = self.apply_undersampling(attention_masks, targets)
    ...
    def run_transformation(self):
        try:
            ...
            train_input_ids, train_attention_masks, train_targets = self.transform_data(train_df, apply_undersampling=True)
